...
contract("Monkey Brothers", (accounts) => {
  ...
  beforeEach(async () => {
    // Clear any pending battle entries
  ...
  describe("Monk Token", () => {
    ...
    it("Transfer MONK between accounts", async () => {
      ...
      const recipientBalance = await monkInstance.checkMonksOf.call(
        accounts[2]
      );
  ...
  describe("Monkey Gacha", () => {
    it("should draw starter pack, receive 3 monkeys", async () => {
      ...
      assert.equal(
        monkeyCount.toString(),
        "3",
        "Player should receive 3 monkeys"
      );
    });
  ...
  describe("Battle Arena", () => {
    it("should allow a player to battle another player", async () => {
      ...
      await monkInstance.giveMonkApproval(gachaInstance.address, 3000, {
        from: player2,
      });
      ...
      const monkey2Id = 1;
      const monkey3Id = 2;
      await arenaInstance.fight([monkey1Id, monkey2Id, monkey3Id], {
        from: player1,
      });
      ...
      const monkey5Id = 5;
      const monkey6Id = 6;
      const result = await arenaInstance.fight(
        [monkey4Id, monkey5Id, monkey6Id],
        { from: player2 }
      );
      try {
