...
contract MonkeyGacha is Ownable {
    ...
    event MonkeyDrawn(address indexed player, uint256 indexed tokenId, uint8 rarity, 
        uint8 powerType, uint256 attack, uint256 health, string name);

    event TokensWithdrawn(address indexed owner, uint256 amount, uint256 timestamp);

     event StatRangeUpdated(uint8 indexed rarity, uint256 minAttack, uint256 maxAttack,
        uint256 minHealth, uint256 maxHealth);

     event RarityOddsUpdated(uint16 commonChance, uint16 rareChance, uint16 epicChance, 
        uint16 legendaryChance);

    constructor(address _monkToken, address _monkeyNFT) {
        require(_monkToken != address(0) && _monkeyNFT != address(0), "Invalid addresses");
        ...
        rarityStats[uint8(Monkeys.Rarity.Legendary)] = StatRange(26, 35, 85, 94);
    ...
    function drawMonkey() external {
        ...
        require(monkToken.balanceOf(msg.sender) >= 1 ether, "Insufficient MONK balance");
        
        // Transfer MONK tokens from player
        require(monkToken.transferFrom(msg.sender, address(this), 1 ether), "Transfer failed");
        ...
        uint256 attack = _random(range.maxAttack - range.minAttack + 1) + range.minAttack;
        uint256 health = _random(range.maxHealth - range.minHealth + 1) + range.minHealth;
        ...
        emit MonkeyDrawn(msg.sender, tokenId, rarity, powerType, attack, health, name);
    }
    ...
    function _determineRarity() internal view returns (uint8) {
        ...
        if (rand < LEGENDARY_CHANCE + EPIC_CHANCE) return uint8(Monkeys.Rarity.Epic);
        if (rand < LEGENDARY_CHANCE + EPIC_CHANCE + RARE_CHANCE) return uint8(Monkeys.Rarity.Rare);
    ...
    function _initializeNames() internal {
        firstNames = ["Sun", "Cloud", "Storm", "Thunder", "Lightning", "Rain", "Wind", "Star"];
        lastNames = ["Warrior", "Guardian", "Protector", "Knight", "Champion", "Hero", "Legend"];
    }

    // View functions
    function getStatRange(uint8 rarity) external view returns (StatRange memory) {
    ...
    function setStatRange(
        ...
        require(rarity <= uint8(Monkeys.Rarity.Legendary), "Invalid rarity");
        require(maxAttack >= minAttack && maxHealth >= minHealth, "Invalid range");
        ...
        emit StatRangeUpdated(rarity, minAttack, maxAttack, minHealth, maxHealth);
    }
    ...
    function updateRarityOdds(
        ...
        require(_commonChance + _rareChance + _epicChance + _legendaryChance == 10000, 
                "Must total 100%");
        ...
        emit RarityOddsUpdated(_commonChance, _rareChance, _epicChance, _legendaryChance);
    }

    function addNames(string[] calldata _firstNames, string[] calldata _lastNames) external onlyOwner {
